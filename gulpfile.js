...
const baseDirs = { // Дирктории для файлов
	default: 'dist', // Директория для разработки, в ней базируется сервер для хотрелоада (Скрипты и стили не сжимаются)
...
const otherFilesExtnames = ['json', 'txt', 'md', 'htaccess'] // Расширения файлов которые не участвуют в сборке, но должны быть в продакшене
...
const dev = { // Функции сборки для разработки
	...
	css: () => {
		return src(path.src.css)
			.pipe(plumber())
			.pipe(sass({outputStyle: 'expanded'})) // компиляция sass,scss в несжатом виде
...
const build = { // функции сборки для продакшена
	...
	css: () => {
		return src(path.src.css)
			.pipe(plumber())
			.pipe(sass({outputStyle: 'compressed'})) // компиляция sass,scss с сжатием
			.pipe(autoprefixer({
...
const server = { // задачи для сервера
	...
	css: () => {
		return src(path.src.css)
			.pipe(plumber())
			.pipe(sass({outputStyle: 'expanded'})) // компиляция sass,scss
